...
async def download_zoom_video_with_playwright(
    zoom_url,
    passcode: str,
    output_filename="zoom_recording.mp4",
):
    async with async_playwright() as p:
        ...
        try:
            ...
            if passcode:
                try:
                    await page.wait_for_selector("#passcode", timeout=5000)
                    print("Passcode field detected, filling in passcode...")
                    await page.fill("#passcode", passcode)
                    await page.click("#passcode_btn")
                    print("Submitted passcode, waiting for recording page...")
                    await page.wait_for_url("**/rec/**", timeout=30000)
                except Exception as e:
                    print(f"No passcode prompt detected or failed to submit: {e}")

            print(
                "Video URL not found immediately. Trying to interact with the page..."
            )
            ...
            if not video_url:
                print(
                    "Can't find video link so far, will try to click 'Watch recording' button"
                )
...
def download_zoom_video(zoom_url, passcode: str, output_filename="zoom_recording.mp4"):
    asyncio.run(
        download_zoom_video_with_playwright(zoom_url, passcode, output_filename)
    )


def main():
    parser = argparse.ArgumentParser(
        description="Download Zoom recordings via command line"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Passcode for the Zoom recording (if required)",
    )
    parser.add_argument(
        "-v", "--version", action="version", version="Zoom Downloader 1.0"
    )

    args = parser.parse_args()

    success = download_zoom_video(args.url, args.passcode, args.output)
